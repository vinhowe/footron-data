...
def setup(w, h, fullscreen=False):
    ...
    # XXX I don't know why, but this line is critical.  I think it somehow initializes torch?
    state = torch.cuda.FloatTensor(1,1,1,1)
...
def torch_draw( frame ):
    tex = screen['tex']
    h,w = tex.shape[:2]

    # copy from torch into buffer
    if tex.nbytes != frame.numel()*frame.element_size():
        print( tex.nbytes )
        print( frame.numel(), frame.element_size() )
        assert False
...
#    print( f"draw time: {time.time()-s0}" )
